...
class BluetoothService:
    ...
    def __init__(self):
        ...
        self._active_device: Optional[BluetoothDevice] = None
        ...
        self._on_incoming_call: Optional[Callable[[CallInfo], None]] = None
        self._on_call_state_changed: Optional[Callable[[CallInfo], None]] = None
        
        logger.info("BluetoothService initialized")
    
    def set_device_connected_callback(self, callback: Callable[[BluetoothDevice], None]):
    ...
    async def disconnect_device(self, address: Optional[str] = None):
        """
        ...
        """
        addr_to_disconnect = address or (self._active_device.address if self._active_device else None)
        ...
        try:
            ...
            if self._active_device and self._active_device.address == addr_to_disconnect:
    ...
    async def answer_call(self) -> CallInfo:
        ...
        if not self._current_call or self._current_call.state != CallState.INCOMING:
            raise BluetoothCallError(self._active_device.name, "answer", "No incoming call")
    ...
    async def make_call(self, phone_number: str, contact_name: Optional[str] = None) -> CallInfo:
        """
    ...
    async def simulate_incoming_call(self, phone_number: str, contact_name: Optional[str] = None):
        """
    ...
    def get_active_device(self) -> Optional[BluetoothDevice]:
