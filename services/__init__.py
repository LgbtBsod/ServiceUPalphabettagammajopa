...
from models.pydantic_models import Order, Client, Device, WorkItem, OrderStatus, Priority
...
class BaseService(Generic[T]):
    ...
    def __init__(self, uow_class: type[UnitOfWork]):
        """
        Инициализация сервиса.
        
        Args:
            uow_class: Класс Unit of Work для транзакций.
        """
        self._uow_class = uow_class

    def _get_repository(self, uow: UnitOfWork, repo_name: str) -> BaseRepository:
        """Получение репозитория из Unit of Work."""
        return getattr(uow, repo_name)
...
class OrderService(BaseService[Order]):
    ...
    def _device_to_order(self, device: Any) -> Order:
        ...
        if hasattr(device, '__dict__') and hasattr(device, '_fields'):
            ...
            device_dict = device._asdict() if hasattr(device, '_asdict') else vars(device)
        ...
        return Order(
            ...
            receipt_date=datetime.fromisoformat(receipt_date_val) if receipt_date_val else datetime.now(),
            ready_date=datetime.fromisoformat(ready_date_val) if ready_date_val else None,
...
def create_services(uow_class: type[UnitOfWork]) -> Dict[str, BaseService]:
    """
    Фабрика для создания сервисов.
    
    Args:
        uow_class: Класс Unit of Work.
        
    Returns:
        Словарь с сервисами.
    """
    return {
        'orders': OrderService(uow_class),
        'clients': ClientService(uow_class),
    }
