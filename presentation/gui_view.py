...
class DashboardGUIView:
    ...
    def _render_widget(self, widget_data: DashboardWidget):
        ...
        frame = ttk.LabelFrame(
            self.scrollable_frame, text=widget_data.title, padding=10
        )
        ...
        if widget_data.widget_type == "kpi_card":
            ...
            ttk.Label(
                frame, text=f"Неизвестный тип виджета: {widget_data.widget_type}"
            ).pack()
    ...
    def _render_chart_placeholder(self, parent, widget_data: DashboardWidget):
        ...
        info_text = "\n".join(
            [
                f"{p['label'] if isinstance(p, dict) else p.label}: {p['value'] if isinstance(p, dict) else p.value}"
                for p in widget_data.data[:5]
            ]
        )
    ...
    def _render_table(self, parent, data: list[dict]):
        ...
        tree = ttk.Treeview(
            parent, columns=cols, show="headings", height=min(len(data), 10)
        )
