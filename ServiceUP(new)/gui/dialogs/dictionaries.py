...
from gui.widgets.premium import PremiumCard, PremiumLabel, PremiumEntry, PremiumButton
...
class DictionariesManagerWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, db, colors: Dict[str, str], settings=None):
        ...
        restore_window_geometry(self.settings, "dictionaries", self,
                                default_w=1000, default_h=650, min_w=900, min_h=550)
    ...
    def create_widgets(self):
        ...
        self.tabview = ctk.CTkTabview(main_container, fg_color=self.colors['bg_primary'])
    ...
    def create_dict_tab(self, parent, dict_type: str, config: dict):
        ...
        tree.bind("<<TreeviewSelect>>", lambda e, dt=dict_type: self.on_item_select(dt, tree))
        ...
        # Поле для значения
        ctk.CTkLabel(form, text="Значение*:", font=ctk.CTkFont(size=12, weight="bold")).pack(anchor="w", pady=(0, 3))
        ...
        ctk.CTkLabel(form, text="Доп. информация:", font=ctk.CTkFont(size=12, weight="bold")).pack(anchor="w", pady=(0, 3))
        ...
        _dict_btns = [
            ("💾 Сохранить", lambda dt=dict_type, ve=value_entry, ie=info_entry: self.save_item(dt, ve, ie),
             self.colors['accent'], "white", self.colors.get('accent_hover'), 0, 0),
            ("➕ Добавить", lambda dt=dict_type, ve=value_entry, ie=info_entry: self.add_new_item(dt, ve, ie),
             self.colors['bg_tertiary'], self.colors['text_primary'], self.colors.get('hover'), 0, 1),
            ("🗑️ Удалить", lambda dt=dict_type: self.delete_item(dt),
             "#d13438", "white", "#b32d30", 1, 0),
            ("✖ Очистить", lambda ve=value_entry, ie=info_entry: self.clear_form(ve, ie),
             self.colors['bg_tertiary'], self.colors['text_primary'], self.colors.get('hover'), 1, 1),
        ]
        for text, cmd, fg, tc, hover, brow, bcol in _dict_btns:
            ctk.CTkButton(
                btn_frame, text=text, command=cmd, height=33, width=130,
                corner_radius=6, fg_color=fg, text_color=tc, hover_color=hover,
    ...
    def save_item(self, dict_type: str, value_entry: ctk.CTkEntry, info_entry: ctk.CTkEntry):
    ...
    def add_new_item(self, dict_type: str, value_entry: ctk.CTkEntry, info_entry: ctk.CTkEntry):
    ...
    def delete_item(self, dict_type: str):
        ...
        if messagebox.askyesno("Подтверждение", "Вы уверены, что хотите удалить этот элемент?"):
