...
_FONT_TTF_MAP = {
    'Trebuchet MS':      (r'C:\Windows\Fonts\trebuc.ttf',   r'C:\Windows\Fonts\trebucbd.ttf',  'TrebuchetMS',      'TrebuchetMS-Bold'),
    'Arial':             (r'C:\Windows\Fonts\arial.ttf',    r'C:\Windows\Fonts\arialbd.ttf',   'ActArial',         'ActArial-Bold'),
    'Times New Roman':   (r'C:\Windows\Fonts\times.ttf',    r'C:\Windows\Fonts\timesbd.ttf',   'ActTimes',         'ActTimes-Bold'),
    'Helvetica':         (r'C:\Windows\Fonts\arial.ttf',    r'C:\Windows\Fonts\arialbd.ttf',   'ActArial',         'ActArial-Bold'),
    # Tahoma и Verdana как альтернативы
    'Tahoma':            (r'C:\Windows\Fonts\tahoma.ttf',   r'C:\Windows\Fonts\tahomabd.ttf',  'ActTahoma',        'ActTahoma-Bold'),
    'Verdana':           (r'C:\Windows\Fonts\verdana.ttf',  r'C:\Windows\Fonts\verdanab.ttf',  'ActVerdana',       'ActVerdana-Bold'),
}
...
def _register_act_font(font_name: str):
    ...
    try:
        ...
        if name_reg in pdfmetrics.getRegisteredFontNames():
            bold = name_bold if name_bold in pdfmetrics.getRegisteredFontNames() else name_reg
...
def _prepare_image(path: str) -> Optional[str]:
...
class ActPDFGenerator:
    ...
    def __init__(self, company_info: Optional[Dict[str, str]] = None,
    ...
    def _save_pdf(self, filepath: str, story: list) -> bool:
        ...
        doc = SimpleDocTemplate(
            ...
            title="Акт", author=self.company.get('name', 'Сервисный центр'),
        )
        doc.build(story)
        return True

    def generate_dual_pdf(self, filepath: str, device1: Dict[str, Any],
                          device2: Dict[str, Any] = None,
                          act_type1: str = "receipt",
                          act_type2: str = "completion") -> bool:
        ...
        try:
            ...
            from reportlab.platypus import SimpleDocTemplate, Spacer, KeepInFrame
            a4_w, a4_h = A4
            doc = SimpleDocTemplate(
                filepath, pagesize=A4,
                leftMargin=self.margin, rightMargin=self.margin,
                topMargin=self.margin, bottomMargin=self.margin,
                title="Акты", author=self.company.get('name', 'Сервисный центр'),
            )
    ...
    def _styles(self):
        ...
        fb = self.font_bold
        return {
            'logo_note': ParagraphStyle('logo_note', fontName=f, fontSize=7, alignment=TA_CENTER, leading=9),
            'company': ParagraphStyle('company', fontName=fb, fontSize=11, alignment=TA_CENTER, leading=13, spaceAfter=1),
            'company_sub': ParagraphStyle('company_sub', fontName=f, fontSize=7.5, alignment=TA_CENTER, leading=9),
            'schedule': ParagraphStyle('schedule', fontName=fb, fontSize=7, alignment=TA_CENTER, leading=8, spaceAfter=2),
            'title': ParagraphStyle('title', fontName=fb, fontSize=11, alignment=TA_CENTER, leading=13, spaceBefore=1, spaceAfter=2),
            'section': ParagraphStyle('section', fontName=fb, fontSize=9, alignment=TA_CENTER, leading=11, spaceBefore=1, spaceAfter=1, backColor=_color("#F0F0F0")),
            'cell_label': ParagraphStyle('cell_label', fontName=f, fontSize=7.5, alignment=TA_RIGHT, leading=9),
            'cell_value': ParagraphStyle('cell_value', fontName=f, fontSize=7.5, alignment=TA_LEFT, leading=9),
            'cell_value_bold': ParagraphStyle('cell_value_bold', fontName=fb, fontSize=9, alignment=TA_LEFT, leading=11),
            'defect': ParagraphStyle('defect', fontName=f, fontSize=7.5, alignment=TA_LEFT, leading=10),
            'footer': ParagraphStyle('footer', fontName=f, fontSize=7, alignment=TA_CENTER, leading=8, spaceBefore=2),
            'sign': ParagraphStyle('sign', fontName=f, fontSize=7, alignment=TA_LEFT, leading=9),
            'th': ParagraphStyle('th', fontName=fb, fontSize=7, alignment=TA_CENTER, leading=8),
            'td': ParagraphStyle('td', fontName=f, fontSize=7, alignment=TA_LEFT, leading=8),
            'td_num': ParagraphStyle('td_num', fontName=f, fontSize=7, alignment=TA_CENTER, leading=8),
            'total_label': ParagraphStyle('total_label', fontName=fb, fontSize=8.5, alignment=TA_RIGHT, leading=10),
            'total_value': ParagraphStyle('total_value', fontName=fb, fontSize=8.5, alignment=TA_CENTER, leading=10),
            'note_text': ParagraphStyle('note_text', fontName=f, fontSize=7, alignment=TA_LEFT, leading=9),
        }
    ...
    def _header_block(self, styles) -> list:
        ...
        story.append(Paragraph(xml_escape(self.company.get('name', '')), styles['company']))
    ...
    def _rows_from_template(self, device: Dict[str, Any], exclude=('order_number',)) -> List[tuple]:
        ...
        special_fields = ('order_number', 'defect', 'total_price', 'prepayment',
                          'completed_work')
        rows = []
        for field in fields:
    ...
    def _build_receipt_story(self, device: Dict[str, Any]) -> list:
        ...
        if show_defect:
            ...
            defect_tbl = Table([[Paragraph(defect_text, styles['defect'])]],
        ...
        if total and show_price:
            ...
            price_tbl = Table([[Paragraph(xml_escape(price_line), styles['defect'])]],
                              colWidths=[A5[0] - 2 * self.margin])
        ...
        if conditions:
            ...
            for line in conditions.split('\n'):
                if line.strip():
                    cond_lines.append([Paragraph(xml_escape(line.strip()), styles['note_text'])])
            if cond_lines:
    ...
    def _build_completion_story(self, device: Dict[str, Any]) -> list:
        ...
        if not rows:
            ...
            rows = [
                ...
                ("Серийный номер", device.get('serial_number', '') or "не указан"),
                ("Дата приема", format_date(device.get('receipt_date', ''))),
                ("Дата выдачи", format_date(completion_date) if completion_date else datetime.now().strftime('%d.%m.%Y')),
        ...
        if warranty_text:
            ...
            for line in warranty_text.split('\n'):
                if line.strip():
                    war_lines.append([Paragraph(xml_escape(line.strip()), styles['note_text'])])
            if war_lines:
                war_tbl = Table(war_lines, colWidths=[A5[0] - 2 * self.margin])
                war_tbl.setStyle(TableStyle([
                    ('LEFTPADDING', (0, 0), (-1, -1), 4),
                    ('TOPPADDING', (0, 0), (-1, -1), 0),
                    ('BOTTOMPADDING', (0, 0), (-1, -1), 0),
                ]))
    ...
    def _works_table(self, device: Dict[str, Any], styles) -> 'Table':
        ...
        if not work_items:
            ...
            if cw:
                ...
                work_items = [{'description': cw, 'quantity': 1, 'price': total, 'total': total}]
        ...
        for item in work_items:
            ...
            data.append([
                ...
                Paragraph(format_price(item['price']).replace(' ₽', ' р.'), styles['td_num']),
                Paragraph(format_price(item['total']).replace(' ₽', ' р.'), styles['td_num']),
            ])
        ...
        data.append([
            ...
            Paragraph(f"{format_price(grand_total).replace(' ₽', '')} р.", styles['total_value']),
        ])
    ...
    def _receipt_text(self, device: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            f"{self.company.get('address', '')}   {self.company.get('phone', '')}".center(60),
...
class PDFRenderer:
    ...
    def export_to_pdf(self, filepath: str, data: Optional[Dict[str, Any]] = None) -> bool:
        data = data or {}
    ...
    def render_act(self, data: Optional[Dict[str, Any]] = None) -> str:
        data = data or {}
        if self.act_type == "completion":
