...
class IntegrationSettingsDialog(ctk.CTkToplevel):
    ...
    def _setup_telegram_tab(self):
        ...
        self.telegram_enabled = ctk.CTkCheckBox(
            tab, text="Включить Telegram уведомления", font=ctk.CTkFont(size=14)
        )
        ...
        self.telegram_token = ctk.CTkEntry(
            tab, width=550, placeholder_text="BotFather token"
        )
        ...
        ctk.CTkLabel(
            tab,
            text="Chat ID (получить через @userinfobot):",
            font=ctk.CTkFont(size=13),
        ).grid(row=3, column=0, padx=30, pady=(10, 5), sticky="w")
        self.telegram_chat_id = ctk.CTkEntry(
            tab, width=550, placeholder_text="Numeric chat ID"
        )
    ...
    def _setup_whatsapp_tab(self):
        ...
        self.whatsapp_enabled = ctk.CTkCheckBox(
            tab, text="Включить WhatsApp уведомления", font=ctk.CTkFont(size=14)
        )
        self.whatsapp_enabled.grid(row=0, column=0, padx=30, pady=(20, 10), sticky="w")

        # API Key
        ctk.CTkLabel(
            tab, text="API Key (Twilio/360dialog):", font=ctk.CTkFont(size=13)
        ).grid(row=1, column=0, padx=30, pady=(10, 5), sticky="w")
        self.whatsapp_api_key = ctk.CTkEntry(
            tab, width=550, placeholder_text="API key from provider"
        )
        ...
        self.whatsapp_phone_id = ctk.CTkEntry(
            tab, width=550, placeholder_text="Business phone number ID"
        )
    ...
    def _setup_vk_tab(self):
        ...
        self.vk_enabled = ctk.CTkCheckBox(
            tab, text="Включить VK уведомления", font=ctk.CTkFont(size=14)
        )
        self.vk_enabled.grid(row=0, column=0, padx=30, pady=(20, 10), sticky="w")

        # Service Key
        ctk.CTkLabel(
            tab, text="Service Key (из настроек сообщества):", font=ctk.CTkFont(size=13)
        ).grid(row=1, column=0, padx=30, pady=(10, 5), sticky="w")
        ...
        # Group ID
        ctk.CTkLabel(
            tab, text="Group ID (сообщество):", font=ctk.CTkFont(size=13)
        ).grid(row=3, column=0, padx=30, pady=(10, 5), sticky="w")
        self.vk_group_id = ctk.CTkEntry(
            tab, width=550, placeholder_text="Numeric group ID"
        )
    ...
    def _setup_email_tab(self):
        ...
        self.email_enabled = ctk.CTkCheckBox(
            tab, text="Включить Email уведомления", font=ctk.CTkFont(size=14)
        )
        ...
        self.email_smtp_server = ctk.CTkEntry(
            tab, width=550, placeholder_text="smtp.gmail.com"
        )
        ...
        self.email_from = ctk.CTkEntry(
            tab, width=550, placeholder_text="your@email.com"
        )
        ...
        self.email_password = ctk.CTkEntry(
            tab, width=550, show="*", placeholder_text="App password"
        )
    ...
    def _create_footer(self):
        ...
        save_btn = ctk.CTkButton(
            btn_frame, text="💾 Сохранить", width=150, command=self._save_settings
        )
        ...
        async def scan():
            ...
            if devices:
                ...
                self.bt_status_label.configure(
                    text=f"✅ Найдено устройств: {len(devices)}", text_color="green"
                )
            else:
                self.bt_status_label.configure(
                    text="❌ Устройства не найдены", text_color="red"
                )
    ...
    def _connect_bluetooth_device(self):
        ...
        async def connect():
            try:
                ...
                self.bt_status_label.configure(
                    text=f"✅ Подключено: {device.name}", text_color="green"
                )
    ...
    def _save_settings(self):
        ...
        settings = {
            ...
            "bluetooth": {
                ...
                "connected_device": self.bt_device_var.get()
                if self.bt_device_var.get() != "Нет устройств"
                else None,
            },
        }
...
def show_integration_settings(
    parent: ctk.CTk, on_save: Callable[[dict[str, Any]], None] | None = None
):
