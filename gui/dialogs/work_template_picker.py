...
from gui.widgets.modern import ModernButton, ModernCard, ModernEntry, ModernLabel
...
class WorkTemplatePicker(ctk.CTkToplevel):
    ...
    def __init__(self, parent, db, colors: dict):
        ...
        self.db = db
        self.colors = colors
        self.result: dict[str, Any] | None = None  # {'description', 'price'}
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(
            main,
            text="🔨 Шаблоны выполненных работ",
        ...
        self.tree = ttk.Treeview(
            tree_container, columns=columns, show="headings", height=14
        )
        ...
        ModernLabel(right_inner, self.colors, text="Название работы:").pack(
            anchor="w", pady=(4, 2)
        )
        self.new_name_entry = ModernEntry(right_inner, self.colors)
        self.new_name_entry.pack(fill="x", pady=(0, 8))

        ModernLabel(right_inner, self.colors, text="Цена (₽):").pack(
            anchor="w", pady=(4, 2)
        )
    ...
    def add_new_template(self):
        ...
        if self.db.add_dict_value("work", name, price_clean):
            ...
            messagebox.showerror(
                "Ошибка", "Не удалось добавить (возможно, уже существует)"
            )
