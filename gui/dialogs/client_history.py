...
class ClientHistoryWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, db: Database, client_db: ClientDatabaseManager,
                 client_name: str, client_phone: str, client_status: str, colors: dict,
                 settings=None):
        ...
        restore_window_geometry(self.settings, "client_history", self,
                                default_w=1200, default_h=700, min_w=950, min_h=560)
    ...
    def _close_with_geometry(self):
        ...
        try:
            ...
            save_window_geometry(self.settings, "client_history", self)
        except Exception:
            pass
        self.destroy()
    ...
    def create_widgets(self):
        ...
        # Аватар клиента
        avatar_frame = ctk.CTkFrame(header_frame, width=60, height=60, corner_radius=30, fg_color=self.colors['accent'])
        avatar_frame.pack(side="left", padx=(0, 15))
        avatar_frame.pack_propagate(False)
        
        avatar_label = ctk.CTkLabel(avatar_frame, text="👤", font=ctk.CTkFont(size=30), text_color="white")
        ...
        ctk.CTkLabel(name_frame, text=self.client_name, font=ctk.CTkFont(size=20, weight="bold"), text_color=self.colors['text_primary']).pack(side="left")
        
        # Статус клиента
        status_colors = {"VIP": "#ffd700", "Постоянный": "#4caf50", "Новый": "#9e9e9e"}
        status_color = status_colors.get(self.client_status, "#9e9e9e")
        status_badge = ctk.CTkFrame(info_frame, fg_color=status_color, corner_radius=10, height=24)
        status_badge.pack(anchor="w", pady=(5, 0))
        status_badge.pack_propagate(False)
        ctk.CTkLabel(status_badge, text=self.client_status, font=ctk.CTkFont(size=11, weight="bold"), text_color="white", padx=10, pady=2).pack()
        
        # Контактная информация
        contact_frame = ctk.CTkFrame(info_frame, fg_color="transparent")
        contact_frame.pack(anchor="w", pady=(8, 0))
        ctk.CTkLabel(contact_frame, text="📞", font=ctk.CTkFont(size=14)).pack(side="left", padx=(0, 5))
        ctk.CTkLabel(contact_frame, text=format_phone(self.client_phone), font=ctk.CTkFont(size=13), text_color=self.colors['text_secondary']).pack(side="left")
        ...
        for i, (icon, label, value, color) in enumerate(stats_items):
            ...
            ctk.CTkLabel(stat_frame, text=icon, font=ctk.CTkFont(size=24), text_color=color).pack(anchor="w")
            ctk.CTkLabel(stat_frame, text=label, font=ctk.CTkFont(size=11), text_color=self.colors['text_secondary']).pack(anchor="w", pady=(2, 0))
            value_label = ctk.CTkLabel(stat_frame, text=value, font=ctk.CTkFont(size=16, weight="bold"), text_color=color)
        ...
        self.search_entry = ModernEntry(search_frame, self.colors, placeholder_text="Поиск по заказам, устройствам...", width=250, height=32)
        ...
        self.status_filter = ModernCombobox(search_frame, self.colors, values=["Все", "Диагностика", "Ожидание запчастей", "В ремонте", "Готов к выдаче", "Выдан клиенту", "Отказ от ремонта"], width=140, height=32)
        ...
        self.year_filter = ModernCombobox(search_frame, self.colors, values=["Все"], width=100, height=32)
        ...
        reset_btn = ModernButton(search_frame, self.colors, variant="secondary", text="Сбросить", command=self.reset_filters, width=80, height=32)
        reset_btn.pack(side="right", padx=(10, 0))
        
        export_btn = ModernButton(search_frame, self.colors, variant="primary", text="📊 Экспорт", command=self.export_history, width=100, height=32)
        ...
        ctk.CTkLabel(table_header, text="📋 История ремонтов", font=ctk.CTkFont(size=15, weight="bold"), text_color=self.colors['accent']).pack(side="left")
        
        self.count_label = ctk.CTkLabel(table_header, text="", font=ctk.CTkFont(size=12), text_color=self.colors['text_secondary'])
        ...
        # Колонки таблицы
        columns = ("Дата", "Заказ №", "Устройство", "Неисправность", "Работы", "Статус", "Цена", "Фото")
        ...
        edit_btn = ctk.CTkButton(menu_frame, text="✏️ Редактировать заказ", command=self.edit_order_from_history, 
                                   fg_color="transparent", text_color=self.colors['text_primary'], 
                                   hover_color=self.colors['bg_hover'], anchor="w", height=28, corner_radius=0, font=ctk.CTkFont(size=12))
        edit_btn.pack(fill="x", padx=4, pady=2)
        
        print_btn = ctk.CTkButton(menu_frame, text="🖨️ Печать акта", command=self.print_act_from_history,
                                 fg_color="transparent", text_color=self.colors['text_primary'], 
                                 hover_color=self.colors['bg_hover'], anchor="w", height=28, corner_radius=0, font=ctk.CTkFont(size=12))
        ...
        ModernButton(close_bar, self.colors, variant="secondary", text="✖ Закрыть",
                     command=self._close_with_geometry, height=35, width=120).pack(side="right")
    ...
    def update_statistics(self):
        ...
        completed = sum(1 for r in self.all_history if r.get('status') == 'Выдан клиенту')
        total_sum = 0
        favorite_device = {}
        ...
        for record in self.all_history:
            ...
            if record.get('status') == 'Выдан клиенту':
                ...
                try:
                    price_val = float(price_str.replace(',', '.').replace(' ', '')) if price_str else 0
                    total_sum += price_val
        ...
        favorite = max(favorite_device.items(), key=lambda x: x[1])[0] if favorite_device else "—"
        ...
        self.stats_labels["Любимое устройство"].configure(text=favorite[:25] + ("..." if len(favorite) > 25 else ""))
    ...
    def filter_history(self):
        ...
        self.count_label.configure(text=f"Показано: {len(filtered)} из {len(self.all_history)}")
    ...
    def load_history(self, history: List[Dict[str, Any]]):
        ...
        for record in history:
            ...
            order_number = format_order_number_for_display(record.get('order_number', ''))
                ...
                try:
                    ...
                    for item in work_data:
                        if item.get('quantity', 1) > 1:
                            work_descriptions.append(f"{item.get('description', '')[:20]} x{item.get('quantity', 1)}")
                        else:
            ...
            price = format_price(record.get('total_price', '')) if record.get('total_price') else '0 ₽'
            ...
            values = (receipt_date, order_number, device_name, defect, work_summary, status_val, price, photo_indicator)
            self.tree.insert("", "end", values=values, tags=tags)
        
        # Настройка цветов для тегов
        self.tree.tag_configure('completed', background=self.colors.get('success_light', '#e6f4ea'))
        self.tree.tag_configure('declined', background=self.colors.get('error_light', '#fce8e8'))
    ...
    def edit_order_from_history(self, event=None):
        ...
        record = None
        for r in self.all_history:
            if str(format_order_number_for_display(r.get('order_number', ''))) == order_number:
                record = r
                break
        ...
        if device:
            ...
            dialog = DeviceFormDialog(self.parent_app, self.db, self.client_db,
                                     self.parent_app.photo_manager if hasattr(self.parent_app, 'photo_manager') else None,
                                     self.colors, is_new=False, device_data=device,
        ...
        if device:
            ...
            if filename and os.path.exists(filename):
                with open(filename, 'r', encoding='utf-8') as f:
                    content = f.read()
                ActPreviewWindow(self, f"Акт приема {order_number}", content, self.colors, "receipt", device, settings=self.settings)
        else:
    ...
    def export_history(self):
        ...
        if file_path:
            with open(file_path, 'w', newline='', encoding='utf-8-sig') as f:
                writer = csv.writer(f)
                writer.writerow(["Дата", "Заказ №", "Устройство", "Неисправность", "Работы", "Статус", "Цена"])
                ...
                for record in self.all_history:
                    ...
                    device_name = f"{record.get('brand', '')} {record.get('model', '')}".strip()
                    defect = record.get('defect', '')
                    work_summary = self.get_work_summary_full(record.get('work_items', ''))
                    status = record.get('status', '')
                    price = format_price(record.get('total_price', ''))
                    
                    writer.writerow([receipt_date, order_number, device_name, defect, work_summary, status, price])
