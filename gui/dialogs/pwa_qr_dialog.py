...
class PWAQRDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, server_url: str, colors: dict,
                 on_stop=None, settings=None):
        ...
        if settings:
            ...
            restore_window_geometry(settings, "pwa_qr", self,
                                   default_w=420, default_h=600, min_w=380, min_h=520)
    ...
    def create_widgets(self):
        ...
        # Заголовок
        ModernLabel(main, self.colors, text="📱 Мобильная версия",
                    font=ctk.CTkFont(size=20, weight="bold"),
                    text_color=self.colors['accent']).pack(pady=(5, 4))

        ModernLabel(main, self.colors,
        ...
        ModernLabel(url_frame, self.colors, text="Адрес:",
                    font=ctk.CTkFont(size=12),
                    text_color=self.colors['text_secondary']).pack(side="left")

        self.url_label = ModernLabel(url_frame, self.colors,
        ...
        ModernLabel(main, self.colors,
                    ...
                    text_color=self.colors['text_secondary'],
                    justify="left").pack(pady=(0, 12), anchor="w", padx=10)
        ...
        if self.on_stop:
            ModernButton(btn_frame, self.colors, variant="danger",
                         text="⏹ Остановить сервер",
                         command=self._stop_server,
                         height=38).pack(side="left", fill="x", expand=True, padx=2)

        ModernButton(btn_frame, self.colors, variant="secondary",
                     text="✖ Закрыть окно",
                     command=self._close,
                     height=38).pack(side="right", padx=2)
