...
from gui.widgets.modern import ModernButton, ModernCard


class ActivationDialog(ctk.CTkToplevel):
    """Диалог активации лицензии."""

    def __init__(self, parent, license_manager, colors: dict | None = None):
    ...
    def create_widgets(self):
        ...
        if status == "activated":
            ...
            header_color = (
                self.colors["accent"]
                if days_left > 3
                else self.colors.get("error", "#ff3b30")
            )
        ...
        ctk.CTkLabel(
            ...
            text_color=self.colors["text_secondary"],
            justify="left",
        ).pack(anchor="w", pady=(5, 8), padx=15)
        ...
        if status == "trial_active":
            ModernButton(
                ...
                self.colors,
                variant="secondary",
                text="Продолжить пробный период",
    ...
    def _try_activate(self):
        ...
        if self.lic.activate(key):
            self.activated = True
            messagebox.showinfo(
                "Успех", "✅ Программа успешно активирована!\nСпасибо за покупку!"
            )
            self.destroy()
        else:
