...
class SettingsWindow(ctk.CTkToplevel):
    ...
    def __init__(self, parent, settings_manager, app):
        ...
        restore_window_geometry(self.settings, "settings", self,
                                default_w=800, default_h=600, min_w=700, min_h=500)
    ...
    def create_general_tab(self):
        ...
        self.confirm_delete_var = ctk.BooleanVar(value=self.settings.get('confirm_delete', True))
        ctk.CTkCheckBox(
            behavior_frame,
        ...
        self.confirm_exit_var = ctk.BooleanVar(value=self.settings.get('confirm_exit', False))
        ctk.CTkCheckBox(
            behavior_frame,
        ...
        self.auto_save_var = ctk.BooleanVar(value=self.settings.get('auto_save_on_close', True))
        ctk.CTkCheckBox(
            behavior_frame,
        ...
        ctk.CTkLabel(defaults_frame, text="Статус по умолчанию:").pack(anchor="w", pady=(0, 3))
        ...
        self.default_status_combo.set(self.settings.get('default_status', 'Диагностика'))
        self.default_status_combo.pack(fill="x", pady=(0, 10))
        
        ctk.CTkLabel(defaults_frame, text="Приоритет по умолчанию:").pack(anchor="w", pady=(0, 3))
        ...
        self.default_priority_combo.set(self.settings.get('default_priority', 'Обычный'))
        self.default_priority_combo.pack(fill="x", pady=(0, 10))
        
        # Уведомления
        notify_frame = self.create_section(scroll, "Уведомления")
        
        self.remind_overdue_var = ctk.BooleanVar(value=self.settings.get('remind_overdue', True))
        ctk.CTkCheckBox(
            notify_frame,
        ...
        self.notify_ready_var = ctk.BooleanVar(value=self.settings.get('notify_on_ready', True))
        ctk.CTkCheckBox(
            notify_frame,
    ...
    def create_appearance_tab(self):
        ...
        self.fullscreen_var = ctk.BooleanVar(value=self.settings.get('fullscreen', False))
        ctk.CTkCheckBox(
            fullscreen_frame,
    ...
    def create_db_tab(self):
        ...
        self.auto_backup_var = ctk.BooleanVar(value=self.settings.get('auto_backup', True))
        ctk.CTkCheckBox(
            backup_frame,
        ...
        ctk.CTkButton(path_row, text="📂 Обзор", command=choose_backup_dir,
                      width=90, height=30, corner_radius=6,
                      fg_color=self.app.colors['bg_tertiary'],
                      text_color=self.app.colors['text_primary']).pack(side="left")
        ...
        self.backup_interval_entry.insert(0, str(self.settings.get('backup_interval', 24)))
        ...
        self.compress_backups_var = ctk.BooleanVar(value=self.settings.get('compress_backups', True))
        ctk.CTkCheckBox(
            backup_frame,
            text="Сжимать архивы",
        ...
        self.create_thumbs_var = ctk.BooleanVar(value=self.settings.get('create_thumbnails', True))
        ctk.CTkCheckBox(
            photos_frame,
    ...
    def create_pwa_tab(self):
        ...
        self.pwa_auto_start_var = ctk.BooleanVar(value=self.settings.get('pwa.auto_start', False))
        ctk.CTkCheckBox(
            server_frame,
        ...
        self.pwa_auto_sync_var = ctk.BooleanVar(value=self.settings.get('pwa.auto_sync', True))
        ctk.CTkCheckBox(
            sync_frame,
        ...
        ctk.CTkLabel(
            interval_frame,
            text="Интервал обновления (секунд):"
        ).pack(side="left")
        self.pwa_sync_interval_entry = ctk.CTkEntry(interval_frame, width=80)
        self.pwa_sync_interval_entry.insert(0, str(self.settings.get('pwa.sync_interval', 30)))
    ...
    def create_section(self, parent, title):
        ...
        ctk.CTkFrame(frame, height=1, fg_color=self.app.colors['border']).pack(fill="x", pady=(0, 8))
    ...
    def choose_color(self):
        """Выбор цвета"""
        color = colorchooser.askcolor(title="Выберите акцентный цвет", initialcolor=self.selected_color)
    ...
    def save_settings(self):
        ...
        try:
            ...
            self.settings.set('overdue_days', self._safe_int(self.overdue_days_entry.get(), 14, lo=1, hi=365))
            ...
            self.settings.set('backup_interval', self._safe_int(self.backup_interval_entry.get(), 24, lo=1, hi=720))
            self.settings.set('backup_count', self._safe_int(self.backup_count_entry.get(), 10, lo=1, hi=100))
            ...
            self.settings.set('pwa.port', self._safe_int(self.pwa_port_entry.get(), 5000, lo=1024, hi=65535))
            self.settings.set('pwa.auto_sync', self.pwa_auto_sync_var.get())
            self.settings.set('pwa.sync_interval', self._safe_int(self.pwa_sync_interval_entry.get(), 30, lo=0, hi=3600))
    ...
    def reset_settings(self):
        ...
        if messagebox.askyesno("Подтверждение", "Сбросить все настройки к значениям по умолчанию?"):
            ...
            messagebox.showinfo("Успех", "Настройки сброшены. Перезапустите программу для применения.")
            self._close_with_geometry()
