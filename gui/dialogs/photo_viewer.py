...
class PhotoViewerWindow(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        self.counter_label = PremiumLabel(
            info_frame,
            text=f"Фото {self.current_index + 1} из {len(self.photos)}"
            if self.photos
            else "Нет фото",
            colors=self.colors,
        ...
        canvas = ctk.CTkCanvas(
            canvas_frame, bg=self.colors["bg_secondary"], highlightthickness=0
        )
        ...
        canvas_window = canvas.create_window(
            (0, 0), window=self.photo_frame, anchor="nw"
        )

        self.photo_label = ctk.CTkLabel(
            self.photo_frame, text="", image=None, fg_color="transparent"
        )
        ...
        if len(self.photos) > 1:
            ...
            self.thumbnails_frame = ctk.CTkFrame(
                thumbnails_card, fg_color="transparent"
            )
    ...
    def load_current_photo(self):
        ...
        try:
            ...
            self.photo_label.configure(
                text="❌ Ошибка загрузки изображения", image=None
            )
