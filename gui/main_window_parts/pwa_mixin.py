...
class PwaMixin:
    ...
    def _start_auto_sync(self):
        ...
        def _tick():
            ...
            # чтобы не сбить выбор пользователя — раньше это утверждалось
            # только в комментарии, самой проверки не было (workflow-
            # найденный баг): все диалоги в этом проекте держат grab_set()
            # (device_form.py, client_history.py и т.п.), поэтому
            # root.grab_current() — надёжный сигнал "что-то модальное открыто",
            # ничего отдельно отслеживать не нужно.
