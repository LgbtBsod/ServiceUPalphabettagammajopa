...
from config import DB_PATH, PHOTOS_DIR, APP_VERSION
from database import Database, ClientDatabaseManager, WorkItemsManager
from managers import SettingsManager, ReportGenerator, BackupManager, IntegrationManager, PhotoManager
from utils.colors import get_colors
from utils.window_effects import apply_window_translucency, apply_rounded_corners
from utils.formatters import (
    format_date, format_phone, format_price, format_order_number_for_display,
    format_order_number_for_db, generate_order_number, get_days_since_receipt
)
...
from gui.dialogs import WorkItemDialog
...
class ServiceCenterApp:
    ...
    def __init__(self):
        ...
        self.colors = get_colors(self.theme, self.settings.get('accent_color', '#0078d4'))
        ...
        self.root.bind('<Escape>', lambda e: self.toggle_fullscreen(False) if self.root.attributes('-fullscreen') else None)
        ...
        self.root.bind('<Return>', lambda e: self.edit_device() if self.tree.selection() else None)
    ...
    def toggle_fullscreen(self, fullscreen: Optional[bool] = None):
    ...
    def refresh_ui(self):
        ...
        current_status = self.status_filter.get() if hasattr(self, 'status_filter') else "Все"
        current_priority = self.priority_filter.get() if hasattr(self, 'priority_filter') else "Все"
        current_device_type = self.device_type_filter.get() if hasattr(self, 'device_type_filter') else "Все"
        current_brand = self.brand_filter.get() if hasattr(self, 'brand_filter') else "Все"
        current_hide = self.hide_completed_var.get() if hasattr(self, 'hide_completed_var') else True
    ...
    def create_widgets(self):
        ...
        """
        self.main_container = ctk.CTkFrame(self.root, fg_color=self.colors['bg_primary'], corner_radius=0)
        ...
        self.dashboard = PremiumDashboard(top_panel, self.db, self.colors,
    ...
    def create_header(self):
        ...
        header_frame = ModernCard(self.main_container, self.colors, height=56, corner_radius=0)
        ...
        logo_frame = ctk.CTkFrame(left_container, fg_color=self.colors['accent'], corner_radius=10, width=36, height=36)
        logo_frame.pack(side="left", padx=(0, 12))
        logo_frame.pack_propagate(False)
        
        logo_label = ctk.CTkLabel(logo_frame, text="🔧", font=ctk.CTkFont(size=18), text_color="white")
        logo_label.pack(expand=True)
        
        title_frame = ctk.CTkFrame(left_container, fg_color="transparent")
        title_frame.pack(side="left")
        
        title_label = ctk.CTkLabel(title_frame, text="ServiceUP", font=ctk.CTkFont(size=18, weight="bold"), text_color=self.colors['text_primary'])
        title_label.pack(anchor="w")

        subtitle_label = ctk.CTkLabel(title_frame, text=f"v{APP_VERSION}", font=ctk.CTkFont(size=11), text_color=self.colors['text_secondary'])
        ...
        editor_btn = ModernButton(right_container, self.colors, variant="secondary", text="📄", command=self.open_report_editor, width=40, height=34)
        ...
        # Кнопка словарей
        dict_btn = ModernButton(right_container, self.colors, variant="secondary", text="📚", command=self.open_dictionaries_manager, width=40, height=34)
        ...
        # Кнопка настроек
        settings_btn = ModernButton(right_container, self.colors, variant="secondary", text="⚙", command=self.open_settings, width=40, height=34)
        ...
        license_btn = ModernButton(right_container, self.colors, variant="secondary", text="🔐", command=self.show_activation, width=40, height=34)
        ...
        self.datetime_label = ctk.CTkLabel(right_container, text="", font=ctk.CTkFont(size=12, weight="bold"), text_color=self.colors['accent'])
    ...
    def create_search_panel(self, parent):
        ...
        search_icon = ctk.CTkLabel(search_container, text="🔍", font=ctk.CTkFont(size=14), text_color=self.colors['text_secondary'])
        search_icon.pack(side="left", padx=(0, 8))
        
        self.search_var = ctk.StringVar()
        
        self.search_entry = ModernEntry(search_container, self.colors, textvariable=self.search_var, placeholder_text="Поиск по заказам, клиентам...", width=220, height=34)
        self.search_entry.pack(side="left", padx=(0, 8))
        self.search_entry.bind("<Return>", lambda e: self.search_devices())
        
        # Кнопка "Найти"
        search_btn = ModernButton(search_container, self.colors, variant="primary", text="🔍 Найти", command=self.search_devices, height=34, width=80)
        search_btn.pack(side="left", padx=(0, 8))

        # Кнопка "Обновить" — перезагружает список заказов из БД
        refresh_btn = ModernButton(search_container, self.colors, variant="secondary", text="🔄 Обновить", command=self.refresh_orders, height=34, width=90)
        refresh_btn.pack(side="left", padx=(0, 8))
        
        add_button = ModernButton(search_container, self.colors, variant="primary", text="➕ Новый заказ", command=self.open_add_device_window, height=34, width=120)
    ...
    def create_devices_table(self, parent):
        ...
        table_title = ModernLabel(header_container, self.colors, text="📋 Список заказов", font=ctk.CTkFont(size=15, weight="bold"))
        ...
        ModernLabel(filter_container, self.colors, text="Статус:").pack(side="left", padx=(0, 8))

        self.status_filter = ModernCombobox(filter_container, self.colors, values=["Все"] + STATUSES, width=140, height=32, command=self._on_filter_changed)
        self.status_filter.set("Все")
        self.status_filter.pack(side="left", padx=(0, 12))

        ModernLabel(filter_container, self.colors, text="Приоритет:").pack(side="left", padx=(0, 8))

        self.priority_filter = ModernCombobox(filter_container, self.colors, values=["Все"] + PRIORITIES, width=110, height=32, command=self._on_filter_changed)
        self.priority_filter.set("Все")
        self.priority_filter.pack(side="left", padx=(0, 12))

        ModernLabel(filter_container, self.colors, text="Тип:").pack(side="left", padx=(0, 8))

        # Значения типа устройства: из словаря + уникальные из заказов
        self.device_type_filter = ModernCombobox(
            filter_container, self.colors,
            values=["Все"] + self._collect_filter_values('device_types', 'device_type'),
            width=130, height=32, command=self._on_filter_changed)
        self.device_type_filter.set("Все")
        self.device_type_filter.pack(side="left", padx=(0, 12))

        ModernLabel(filter_container, self.colors, text="Бренд:").pack(side="left", padx=(0, 8))

        self.brand_filter = ModernCombobox(
            filter_container, self.colors,
            values=["Все"] + self._collect_filter_values('brands', 'brand'),
            width=120, height=32, command=self._on_filter_changed)
        self.brand_filter.set("Все")
        self.brand_filter.pack(side="left", padx=(0, 12))
        
        self.hide_completed_var = ctk.BooleanVar(value=self.settings.get('show_completed', True))
        hide_completed_check = ModernSwitch(filter_container, self.colors, text="Скрыть выполненные", variable=self.hide_completed_var, command=self.toggle_completed)
        ...
        ModernButton(quick_frame, self.colors, variant="outline", text="📅 Сегодня",
                     command=self.show_today_orders, height=30, width=85).pack(side="left", padx=2)
        ModernButton(quick_frame, self.colors, variant="outline", text="📆 Неделя",
                     command=self.show_week_orders, height=30, width=85).pack(side="left", padx=2)
        ModernButton(quick_frame, self.colors, variant="outline", text="⏰ Просроч.",
                     command=self.show_overdue_orders, height=30, width=95).pack(side="left", padx=2)
        
        tree_container = ctk.CTkFrame(table_card, fg_color="transparent")
        tree_container.pack(fill="both", expand=True, padx=16, pady=(0, 12))
        
        columns = ("Заказ №", "Дата приёма", "Дней", "Дата выдачи", "Устройство", "Клиент", "Телефон",
                  "Неисправность", "Статус", "Приоритет", "Инженер", "Цена", "Фото")

        style = ttk.Style()
        style.theme_use("clam")
        style.configure("Treeview", background=self.colors['bg_secondary'], foreground=self.colors['text_primary'], fieldbackground=self.colors['bg_secondary'], borderwidth=0, font=('Segoe UI', 11))
        style.configure("Treeview.Heading", background=self.colors['bg_tertiary'], foreground=self.colors['text_primary'], borderwidth=0, font=('Segoe UI', 11, 'bold'))
        ...
        default_widths = {
            "Заказ №": 60, "Дата приёма": 90, "Дней": 45, "Дата выдачи": 80,
            "Устройство": 170, "Клиент": 140, "Телефон": 110,
            "Неисправность": 170, "Статус": 110, "Приоритет": 90,
            "Инженер": 100, "Цена": 80, "Фото": 50,
        }
        ...
        self.tree.tag_configure('urgent', background=self.colors.get('urgent_light', '#4a1e1e'))
        self.tree.tag_configure('warning', background=self.colors.get('warning_light', '#4a3a1e'))
        self.tree.tag_configure('normal', background=self.colors.get('normal_light', self.colors['bg_secondary']))
    ...
    def create_finance_tab(self, parent):
        ...
        ctk.CTkLabel(period_frame, text="Период:", font=ctk.CTkFont(size=12, weight="bold")).pack(side="left", padx=(0, 10))
        ...
        for text, value in periods:
            btn = ctk.CTkRadioButton(
                ...
                command=self.update_finance_display, fg_color=self.colors['accent']
            )
        ...
        self.income_label = ctk.CTkLabel(stats_frame, text="💰 Доход: 0 ₽", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.colors['success'])
        self.income_label.pack(side="left", padx=(0, 20))
        
        self.expense_label = ctk.CTkLabel(stats_frame, text="💸 Расход: 0 ₽", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.colors['error'])
        self.expense_label.pack(side="left", padx=(0, 20))
        
        self.profit_label = ctk.CTkLabel(stats_frame, text="📈 Прибыль: 0 ₽", font=ctk.CTkFont(size=14, weight="bold"), text_color=self.colors['accent'])
        ...
        ctk.CTkLabel(table_card, text="📊 Список завершённых заказов", font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=15, pady=(10, 5))
        ...
        # Стиль для таблицы
        style = ttk.Style()
        style.configure("Finance.Treeview", background=self.colors['bg_secondary'], foreground=self.colors['text_primary'], 
                       fieldbackground=self.colors['bg_secondary'], borderwidth=0, font=('Segoe UI', 11), rowheight=30)
        style.configure("Finance.Treeview.Heading", background=self.colors['bg_tertiary'], foreground=self.colors['text_primary'], 
                       borderwidth=0, font=('Segoe UI', 11, 'bold'))
        
        columns = ("Заказ №", "Дата выдачи", "Доход", "Расход", "Прибыль")
        
        self.finance_tree = ttk.Treeview(tree_container, columns=columns, show="headings", height=15, style="Finance.Treeview")
    ...
    def update_finance_display(self):
        ...
        self.income_label.configure(text=f"💰 Доход: {format_price(str(summary['total_income']))}")
        self.expense_label.configure(text=f"💸 Расход: {format_price(str(summary['total_expense']))}")
        self.profit_label.configure(text=f"📈 Прибыль: {format_price(str(summary['total_profit']))}")
        ...
        for finance in finances:
            order_number = format_order_number_for_display(finance.get('order_number', '_'))
            ...
            self.finance_tree.insert("", "end", values=(order_number, completion_date, income, expense, profit), tags=tags)
        
        # Настройка цветов
        self.finance_tree.tag_configure('positive', background=self.colors.get('success_light', '#e6f4ea'))
        self.finance_tree.tag_configure('negative', background=self.colors.get('error_light', '#fce8e8'))
    ...
    def edit_expense(self, event):
        ...
        ctk.CTkLabel(dialog, text=f"Заказ №{order_number_display}", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=10)
        
        ctk.CTkLabel(dialog, text="Доход:").pack(anchor="w", padx=20)
        income_label = ctk.CTkLabel(dialog, text=format_price(str(finance_record.get('income', 0))), font=ctk.CTkFont(size=12))
        ...
        ctk.CTkButton(btn_frame, text="Сохранить", command=save_expense, width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Отмена", command=dialog.destroy, width=100).pack(side="left", padx=5)
        ...
        for text, command in menu_items:
            ...
            def _make_wrapper(cmd):
                def _wrapper():
                    ...
                    cmd()
                return _wrapper
            btn = ctk.CTkButton(menu_frame, text=text, command=_make_wrapper(command), fg_color="transparent", text_color=self.colors['text_primary'], hover_color=self.colors['bg_hover'], anchor="w", height=32, corner_radius=0, font=ctk.CTkFont(size=12))
    ...
    def _create_action_panel_in(self, parent):
        ...
        for text, command in action_buttons:
            btn = ModernButton(buttons_container, self.colors, variant="secondary", text=text, command=command, height=34, width=150)
    ...
    def toggle_mobile_server(self):
        ...
        try:
            ...
            if not self.pwa_manager.is_running:
                ...
                if not started:
                    messagebox.showerror("Ошибка", "Не удалось запустить сервер мобильной версии.\n"
                                                    "Проверьте, что порт свободен.")
                    return
                # Небольшая пауза для старта Flask
                self.root.after(500, lambda: self._show_qr_dialog())
    ...
    def _show_qr_dialog(self):
        ...
        if not self.pwa_manager or not self.pwa_manager.is_running:
            return
        url = self.pwa_manager.get_url()
        PWAQRDialog(self.root, url, self.colors,
    ...
    def _create_status_bar_in(self, parent):
        ...
        self.status_bar = ctk.CTkFrame(parent, fg_color=self.colors['bg_secondary'], height=28, corner_radius=0)
        ...
        self.status_label = ModernLabel(left_status, self.colors, text="✅ Готово", font=ctk.CTkFont(size=11), text_color=self.colors['success'])
        self.status_label.pack(side="left")
        
        separator = ctk.CTkFrame(left_status, width=1, height=14, fg_color=self.colors['border'])
        separator.pack(side="left", padx=10)
        
        self.db_status = ModernLabel(left_status, self.colors, text="💾 БД активна", font=ctk.CTkFont(size=11), text_color=self.colors['success'])
        ...
        version_label = ModernLabel(right_status, self.colors, text=f"v{APP_VERSION}", font=ctk.CTkFont(size=10), text_color=self.colors['text_tertiary'])
    ...
    def _render_device_row(self, device: Dict[str, Any]) -> tuple:
        ...
        completion_date = (
            ...
            if device.get('completion_date') else ''
        )
        ...
        values = (
            ...
            priority_icon,
            device.get('engineer', ''),
            format_price(device.get('total_price', '')) if device.get('total_price') else '',
            photo_indicator,
        )
        return values, tag
    ...
    def apply_filters(self):
        ...
        try:
            ...
            device_type = self.device_type_filter.get() if hasattr(self, 'device_type_filter') else "Все"
    ...
    def show_overdue_orders(self):
        ...
        try:
            ...
            overdue = [d for d in all_devices if get_days_since_receipt(d.get('receipt_date', '')) > 14]
    ...
    def show_today_orders(self):
        ...
        try:
            ...
            today_orders = [d for d in all_devices if (d.get('receipt_date', '') or '')[:10] == today]
    ...
    def sort_treeview(self, col):
        ...
        if col == "Цена":
            items.sort(key=lambda x: float(x[0].replace('₽', '').replace(' ', '').replace(',', '')) if x[0] != '0 ₽' else 0, reverse=self.sort_reverse)
        else:
        ...
        for heading in self.tree['columns']:
            if heading == col:
                self.tree.heading(heading, text=f"{heading} {'▼' if self.sort_reverse else '▲'}")
            else:
                self.tree.heading(heading, text=heading)
    
    def get_device_id_by_order_number(self, order_number_display: str) -> Optional[int]:
    ...
    def edit_device(self):
        ...
        if not device_id:
            messagebox.showerror("Ошибка", f"Не удалось найти устройство с номером {order_number_display}")
            return
    ...
    def open_add_device_window(self):
        ...
        dialog = DeviceFormDialog(self.root, self.db, self.client_db, self.photo_manager, self.colors, is_new=True, settings=self.settings)
        ...
        if dialog.result:
            ...
            self.dashboard.update_stats()
            self.update_finance_display()
            self.update_status_bar(f"Создан заказ: #{format_order_number_for_display(dialog.result.get('order_number', ''))}")
    
    def open_edit_device_window(self, device_data: Dict[str, Any]):
        """Открытие окна редактирования заказа"""
        dialog = DeviceFormDialog(self.root, self.db, self.client_db, self.photo_manager, self.colors, is_new=False, device_data=device_data, settings=self.settings)
        self.root.wait_window(dialog)
    ...
    def print_receipt_act(self):
        ...
        try:
            ...
            if filename and os.path.exists(filename):
                ...
                ActPreviewWindow(self.root, f"Акт приема {order_number}", content, self.colors, "receipt", device, template, settings=self.settings)
            else:
                messagebox.showerror("Ошибка", "❌ Не удалось создать файл акта")
                    
        except Exception as e:
            messagebox.showerror("Ошибка", f"❌ Ошибка создания документа: {str(e)}")
    ...
    def print_completion_act(self):
        ...
        try:
            ...
            if filename and os.path.exists(filename):
                ...
                ActPreviewWindow(self.root, f"Акт выполненных работ {order_number}", content, self.colors, "completion", device, template, settings=self.settings)
                ...
                if device.get('status') != 'Выдан клиенту':
                    if messagebox.askyesno(
                        ...
                        if self.db.update_device_status(device_id, 'Выдан клиенту'):
                            self.load_devices()
                            self.dashboard.update_stats()
                            self.update_finance_display()
    ...
    def _ask_dual_print_mode(self):
        ...
        ctk.CTkLabel(dialog, text="📋 Выберите режим печати",
                     font=ctk.CTkFont(size=16, weight="bold"),
                     text_color=self.colors['accent']).pack(pady=(20, 15))

        ctk.CTkLabel(dialog, text="Что напечатать на одном листе A4?",
        ...
        ModernButton(dialog, self.colors, variant="primary",
                     text="📄📄  2× Акт приёма",
                     command=lambda: _choose('receipt2'),
                     height=42, width=320).pack(pady=4)
        ...
        ModernButton(dialog, self.colors, variant="secondary",
        ...
        ModernButton(dialog, self.colors, variant="secondary",
                     ...
                     height=36, width=320).pack(pady=(12, 4))
    ...
    def print_dual_acts(self):
        ...
        try:
            ...
            if ok:
                messagebox.showinfo("Успех", f"Два акта на листе A4 сохранены:\n{file_path}")
    ...
    def show_client_history(self):
        ...
        if client_name and client_phone:
            ...
            ClientHistoryWindow(self.root, self.db, self.client_db, client_name, client_phone, client_status, self.colors, settings=self.settings)
    ...
    def open_dictionaries_manager(self):
        ...
        DictionariesManagerWindow(self.root, self.db, self.colors, settings=self.settings)
    ...
    def open_report_editor(self):
        ...
        try:
            ...
            messagebox.showerror("Ошибка", f"Не удалось загрузить редактор: {e}\n\nУстановите reportlab: pip install reportlab")
