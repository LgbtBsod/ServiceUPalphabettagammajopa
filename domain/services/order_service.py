...
from ..entities import Device, OrderStatus, Priority
from ..aggregates import OrderAggregate
from ..events import OrderCreatedEvent, OrderStatusChangedEvent, OrderCompletedEvent
...
class EventDispatcherProtocol(Protocol):
    ...
    def dispatch(self, event: OrderCreatedEvent | OrderStatusChangedEvent | OrderCompletedEvent) -> None: ...
...
class OrderService:
    ...
    ) -> Tuple[bool, Optional[OrderAggregate], str]:
        """
    ...
    def update_status(
        ...
        try:
            ...
            if not aggregate.update_status(new_status):
                return False, f"Недопустимый переход статуса: {old_status} -> {new_status}"
    ...
    def get_statistics(self, days: int = 30) -> dict:
        ...
        in_progress = sum(1 for d in devices if d.status in [OrderStatus.DIAGNOSTICS, OrderStatus.IN_PROGRESS])
        ...
        if completed_devices:
            avg_repair_time = sum(d.days_in_repair for d in completed_devices) / len(completed_devices)
