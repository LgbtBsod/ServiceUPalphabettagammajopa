    ...
    def send_notification(
        ...
        for channel_name in channel_names:
            ...
            try:
                ...
                if success:
                    logger.info(
                        f"Уведомление отправлено через {channel_name}: {recipient}"
                    )
                else:
                    logger.error(
                        f"Не удалось отправить уведомление через {channel_name}"
                    )

            except Exception as e:
                logger.error(
                    f"Ошибка отправки через {channel_name}: {e}", exc_info=True
                )
                results[channel_name] = False
