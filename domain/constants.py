...
DICTIONARY_TYPES: Final[dict[str, dict]] = {
    ...
    "defects": {
        "name": "Неисправности",
        "icon": "🔧",
        # Плоский список (НЕ scoped_by, в отличие от "models") — типовые
        # неисправности не зависят от бренда так же жёстко, как модели, и
        # общий список остаётся обозримым. См. DeviceDefectRecord
        # (database/sqlalchemy_models.py) — теги-неисправности устройства
        # дополняют devices.defect (свободный текст), не заменяют его;
        # ручной ввод тега вне этого справочника допустим и в будущей
        # аналитике агрегируется в общую группу "Прочее".
        "default_values": [
            "Не включается",
            "Не заряжается",
            "Разбит экран",
            ...
            "Программный сбой",
            "Не читает SIM/карту памяти",
            "Шумит/не крутится кулер",
    ...
    "order_tags": {
        "name": "Теги заказа",
        "icon": "🏷",
        # Плоский список, как "defects" — общая метка заказа (не привязана
        # к бренду/модели/неисправности), из справочника или введённая
        # вручную (тот же паттерн). НЕ путать с "defects": defects — это
        # что сломано у устройства, order_tags — произвольная классификация
        # самого заказа (VIP-клиент, срочность, повторное обращение и т.п.),
        # см. OrderTagRecord (database/sqlalchemy_models.py).
        "default_values": [
            "VIP",
            "Срочно",
            "Постоянный клиент",
