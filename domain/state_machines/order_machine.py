...
class OrderStateMachine:
    ...
    STATES = [
        ...
        State(name=OrderStatus.WAITING_APPROVAL.value, on_enter='_on_enter_waiting_approval'),
    ...
    TRANSITIONS = [
        # From DRAFT
        {'trigger': 'create_order', 'source': OrderStatus.DRAFT.value, 'dest': OrderStatus.NEW.value},
        
        # From NEW
        {'trigger': 'start_diagnostics', 'source': OrderStatus.NEW.value, 'dest': OrderStatus.DIAGNOSTICS.value},
        {'trigger': 'cancel_order', 'source': OrderStatus.NEW.value, 'dest': OrderStatus.CANCELLED.value},
        
        # From DIAGNOSTICS
        {'trigger': 'approve_estimate', 'source': OrderStatus.DIAGNOSTICS.value, 'dest': OrderStatus.WAITING_PARTS.value},
        {'trigger': 'request_client_approval', 'source': OrderStatus.DIAGNOSTICS.value, 'dest': OrderStatus.WAITING_APPROVAL.value},
        {'trigger': 'cancel_order', 'source': OrderStatus.DIAGNOSTICS.value, 'dest': OrderStatus.CANCELLED.value},
        
        # From WAITING_APPROVAL
        {'trigger': 'client_approved', 'source': OrderStatus.WAITING_APPROVAL.value, 'dest': OrderStatus.WAITING_PARTS.value},
        {'trigger': 'client_rejected', 'source': OrderStatus.WAITING_APPROVAL.value, 'dest': OrderStatus.CANCELLED.value},
        
        # From WAITING_PARTS
        {'trigger': 'parts_received', 'source': OrderStatus.WAITING_PARTS.value, 'dest': OrderStatus.IN_PROGRESS.value},
        {'trigger': 'cancel_order', 'source': OrderStatus.WAITING_PARTS.value, 'dest': OrderStatus.CANCELLED.value},
        
        # From IN_PROGRESS
        {'trigger': 'complete_repair', 'source': OrderStatus.IN_PROGRESS.value, 'dest': OrderStatus.TESTING.value},
        
        # From TESTING
        {'trigger': 'pass_testing', 'source': OrderStatus.TESTING.value, 'dest': OrderStatus.READY.value},
        {'trigger': 'fail_testing', 'source': OrderStatus.TESTING.value, 'dest': OrderStatus.IN_PROGRESS.value},
        
        # From READY
        {'trigger': 'deliver_to_client', 'source': OrderStatus.READY.value, 'dest': OrderStatus.CLOSED.value},
        
        # From any state to CANCELLED (except already terminal)
        {'trigger': 'cancel_order', 'source': '*', 'dest': OrderStatus.CANCELLED.value, 
    ...
    def _register_transition_logger(self):
        ...
        for transition in self.TRANSITIONS:
            ...
            self.machine.get_transitions(trigger=trigger, source=transition['source'])[0].before.append(
    ...
    def __repr__(self) -> str:
        return f"OrderStateMachine(order_id={self.order_id}, status={self.current_status})"
