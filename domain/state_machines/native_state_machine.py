...
from typing import Protocol, Dict, List, Optional, Set
import logging
...
class NativeStateMachine:
    ...
    ALLOWED_TRANSITIONS: Dict[OrderStatus, Set[OrderStatus]] = field(default_factory=lambda: {
        OrderStatus.DRAFT: {OrderStatus.NEW, OrderStatus.CANCELLED},
        OrderStatus.NEW: {OrderStatus.DIAGNOSTICS, OrderStatus.CANCELLED},
        OrderStatus.DIAGNOSTICS: {OrderStatus.WAITING_PARTS, OrderStatus.REPAIRING, OrderStatus.REFUSED},
    ...
    def transition_to(
        ...
        for callback in self.callbacks:
            try:
                ...
                logger.error(f"Ошибка callback при переходе состояния: {e}")
...
_order_state_machine: Optional[NativeStateMachine] = None


def get_order_state_machine(initial_status: OrderStatus = OrderStatus.DRAFT) -> NativeStateMachine:
