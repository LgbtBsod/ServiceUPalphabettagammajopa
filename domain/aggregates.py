...
from .entities import Device, Client, WorkItem, Photo, RepairHistory, FinanceRecord
...
class OrderAggregate:
    ...
    def update_status(self, new_status: str) -> bool:
        ...
        allowed_transitions = {
            OrderStatus.DIAGNOSTICS: [OrderStatus.WAITING_PARTS, OrderStatus.IN_PROGRESS, OrderStatus.CANCELLED],
    ...
    def add_photo(self, file_path: str, filename: str, photo_type: str = "device") -> None:
    ...
    def to_finance_record(self) -> Optional[FinanceRecord]:
    ...
    def can_be_issued(self) -> bool:
        ...
        return (
            self.device.status == OrderStatus.READY and
            self.device.total_price > 0 and
            self.device.client_name.strip() and
            self.device.phone.strip()
        )
