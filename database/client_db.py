...
class ClientDatabaseManager:
    ...
    def create_client_database(self, client_name: str, client_phone: str) -> Optional[str]:
    ...
    def add_repair_to_client_history(self, client_name: str, client_phone: str,
                                      device_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if self._main_db is not None:
                try:
                    ...
                    if client_id:
                        ...
                        cursor.execute('SELECT id FROM devices WHERE order_number = ?', (order_number,))
            ...
            order_number = device_data.get('order_number', '')
            cursor.execute('SELECT id FROM repair_history WHERE order_number = ?', (order_number,))
                ...
                if is_update:
                    ...
                    cursor.execute('''
                        UPDATE client_stats SET
                            ...
                            updated_at = CURRENT_TIMESTAMP
                        WHERE id = 1
                    ''', (status, status, price_val,
                          datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                          photos_count, favorite_device))
            else:
                cursor.execute('''
    ...
    def update_repair_in_history(self, client_name: str, client_phone: str, 
                                  order_number: str, device_data: Dict[str, Any]) -> bool:
    ...
    def get_client_stats(self, client_name: str, client_phone: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats_dict.get('completed_orders', 0) > 0:
                stats_dict['avg_price'] = stats_dict.get('total_spent', 0) / stats_dict.get('completed_orders', 1)
            else:
