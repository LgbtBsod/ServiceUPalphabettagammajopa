...
class ITokenManager:
    ...
    def generate_token(self, user_id: int, permissions: List[Permission], 
                       expires_in_days: int) -> ApiToken:
...
class AuthService(BaseService):
    ...
    def login(self, command: LoginCommand) -> Optional[SessionEntity]:
        ...
        try:
            ...
            if user.locked_until and user.locked_until > datetime.now():
                self.logger.warning(f"Account locked: {command.username} until {user.locked_until}")
                return None
...
class UserService(BaseService):
    ...
    def create_user(self, command: CreateUserCommand) -> Optional[UserEntity]:
        """Create a new user."""
        try:
    ...
    def change_password(self, command: ChangePasswordCommand) -> bool:
        ...
        try:
            ...
            # Verify old password
            if not self._password_hasher.verify(command.old_password, user.password_hash):
    ...
    def _get_role_permissions(self, role: UserRole) -> Set[Permission]:
        ...
        role_permissions = {
            ...
            UserRole.MANAGER: {
                ...
                Permission.REPORT_GENERATE, Permission.REPORT_PRINT,
            ...
            UserRole.VIEWER: {
                Permission.ORDER_READ, Permission.CLIENT_READ,
                Permission.REPORT_GENERATE
            }
        }
...
class AuthPlugin(IPlugin):
    ...
    def __init__(self):
        ...
        self._session_repo: Optional[ISessionRepository] = None
