...
> **⚠️ УСТАРЕЛО (см. AUDIT_REPORT_v21.md):** Описанная здесь File-Wrapper
> система (`modules/`, автозагрузка из этой папки) удалена этой сессией как
> мёртвый код — её собственный пример модуля не находился её же правилами
> обнаружения. Живая plugin-система — `core/plugin_system.py` + `plugins/`.
