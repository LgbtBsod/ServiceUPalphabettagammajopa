...
class EventTypeFilter(EventFilter):
    ...
    def __init__(self, event_types: Union[str, List[str]]):
        self.event_types = {event_types} if isinstance(event_types, str) else set(event_types)
...
class EventBus(LoggableMixin):
    ...
    def subscribe(
        ...
        """
        event_type_str = event_type if isinstance(event_type, str) else event_type.__name__
        ...
        self.logger.debug(f"Subscribed handler {handler.__name__} to event {event_type_str}")
        ...
        for subscription in subscribers:
            try:
                ...
                if subscription.is_async:
                    ...
                    asyncio.create_task(self._call_async_handler(subscription.handler, event))
                else:
                    # Синхронный обработчик
                    subscription.handler(event)
                    
            except Exception as e:
                self.logger.error(f"Error in event handler {subscription.handler.__name__}: {e}")
    ...
    async def publish_async(self, event: Event) -> None:
        ...
        for subscription in subscribers:
            try:
                ...
                self.logger.error(f"Error preparing event handler: {e}")
                self._add_to_dead_letter(event, subscription, e)
...
def on_event(event_type: Union[str, Type[Event]], priority: int = EventPriority.NORMAL):
...
def async_on_event(event_type: Union[str, Type[Event]], priority: int = EventPriority.NORMAL):
