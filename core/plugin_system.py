...
class PluginManager(LoggableMixin):
    ...
    def unregister(self, plugin_name: str) -> None:
        ...
        if self._states[plugin_name] != PluginState.UNLOADED:
            raise PluginError(f"Plugin '{plugin_name}' must be unloaded before unregistering")
    ...
    def load(self, plugin_name: str) -> bool:
        ...
        for dep in plugin.metadata.dependencies:
            if dep not in self._plugins or self._states.get(dep) != PluginState.ACTIVE:
                self.logger.error(f"Plugin '{plugin_name}' requires '{dep}' which is not active")
    ...
    def enable(self, plugin_name: str) -> bool:
        ...
        if self._states[plugin_name] != PluginState.UNLOADED:
            self.logger.warning(f"Plugin '{plugin_name}' is in {self._states[plugin_name]} state")
            return False
    ...
    def get_api(self, plugin_name: str) -> Optional[Any]:
