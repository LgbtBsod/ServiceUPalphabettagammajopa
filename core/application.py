...
# from core.events import Event, EventBus  # Lazy import to avoid circular dependency
# from domain.state_machines.order_machine import OrderStateMachine
# from application.notifications.notification_hub import NotificationHub
...
class CoreApplication:
    ...
    def __init__(self) -> None:
        ...
        self._container: Optional[containers.Container] = None
    ...
    @inject
    async def initialize(self, container: containers.Container = Provide['container']) -> None:
        """
        ...
        for idx, (stage_name, description) in enumerate(stages, 1):
            ...
            try:
                ...
                logger.error(f"Failed to initialize stage '{stage_name}': {e}")
    ...
    async def _run_stage(self, stage_name: str, description: str) -> None:
        ...
        if stage_name == "config":
            ...
            i18n = self._container.i18n_service() if hasattr(self._container, 'i18n_service') else None
            if i18n:
                await i18n.load_all_languages()
        elif stage_name == "database":
            db = self._container.database() if hasattr(self._container, 'database') else None
            if db:
                await async_wrap(db.connect)()
        elif stage_name == "cache":
            cache = self._container.cache_service() if hasattr(self._container, 'cache_service') else None
            if cache:
                await cache.warm_up()
...
_app_instance: Optional[CoreApplication] = None
