...
class ModuleInfo:
    ...
    wrapper_file: Path | None = None  # Путь к файлу-обертке
    impl_path: Path | None = None  # Путь к реализации
...
class ModuleBase:
    ...
    def __init__(self, app_container=None, module_info: ModuleInfo | None = None):
...
class ModuleRegistry:
    ...
    def discover_modules(self) -> list[str]:
        ...
        for wrapper_file in self.modules_path.glob("module_*.py"):
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    f"modules.{wrapper_file.stem}", wrapper_file
                )
                ...
                if module_class is None:
                    logger.warning(
                        f"No ModuleBase subclass found in {wrapper_file.name}"
                    )
                    continue

                # Читаем метаданные из класса или файла
                name = getattr(
                    module_class, "name", wrapper_file.stem.replace("module_", "")
                )
                ...
                logger.info(
                    f"Discovered wrapper module: {name} v{version} (impl: {impl_path.name if impl_path else 'N/A'})"
                )

            except Exception as e:
                logger.exception(
                    f"Failed to discover wrapper module {wrapper_file.name}: {e}"
                )
        ...
        for item in self.modules_path.iterdir():
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    f"modules.{item.name}", module_file
                )
                if spec is None or spec.loader is None:
                    continue
        ...
        for name in sorted_modules:
            ...
            try:
                ...
                instance = module_class(
                    app_container=app_container, module_info=module_info
                )
    ...
    def get_module(self, name: str) -> ModuleBase | None:
...
def get_module_registry(modules_path: Path | None = None) -> ModuleRegistry:
