...
class SkeletonFrame(ctk.CTkFrame if ctk else tk.Frame):  # type: ignore
    ...
    def __init__(
        ...
        stages: list[LoadingStage] | None = None,
        width: int = 400,
        height: int = 300,
        **kwargs,
    ) -> None:
        ...
        if ctk:
            self.progress_bar = ctk.CTkProgressBar(
                self, mode="determinate", progress_color="#3b8ed0", fg_color="#3a3a3a"
            )
            ...
            self.progress_canvas = tk.Canvas(
                self, height=10, bg="#3a3a3a", highlightthickness=0
            )
            self.progress_canvas.pack(padx=20, pady=(10, 5), fill=tk.X)
            self.progress_fill = self.progress_canvas.create_rectangle(
                0, 0, 0, 10, fill="#3b8ed0", outline=""
            )
    ...
    def _create_stage_widgets(self) -> None:
        ...
        for i, stage in enumerate(self.stages):
            frame = (
                ctk.CTkFrame(self.stages_frame, fg_color="transparent")
                if ctk
                else tk.Frame(self.stages_frame, bg="#2b2b2b")
            )
            ...
            if ctk:
                status_label = ctk.CTkLabel(
                    frame, text="●", text_color=status_color, font=ctk.CTkFont(size=14)
                )
            else:
                status_label = tk.Label(
                    frame, text="●", fg=status_color, bg="#2b2b2b", font=("Arial", 14)
                )
            ...
            if ctk:
                name_label = ctk.CTkLabel(
                    ...
                    font=ctk.CTkFont(
                        size=13,
                        weight="bold" if i == self.current_stage_index else "normal",
                    ),
                    anchor="w",
                )
            else:
                name_label = tk.Label(
                    ...
                    font=(
                        "Arial",
                        13,
                        "bold" if i == self.current_stage_index else "normal",
                    ),
                    anchor="w",
                )
    ...
    def update_progress(
        self, current: int, total: int, stage_name: str, details: str | None = None
    ) -> None:
...
class BusyIndicator(ctk.CTkCanvas if ctk else tk.Canvas):  # type: ignore
    ...
    def _draw_spinner(self) -> None:
        ...
        self.angle + 90  # Дуга в 90 градусов
...
class LoadingOverlay(ctk.CTkToplevel if ctk else tk.Toplevel):  # type: ignore
    ...
    def update_progress(
        self, current: int, total: int, stage: str, details: str | None = None
    ) -> None:
        """Обновление прогресса загрузки."""
            ...
            def load():
                for i, stage in enumerate(stages, 1):
                    overlay.update_progress(
                        i, len(stages), stage.name, stage.description
                    )
