...
class ActPanel:
    ...
    def __init__(self, master_tab, editor, act_type: str):
        ...
        self.left_scroll = ctk.CTkScrollableFrame(
            master_tab, fg_color="transparent", width=420, label_anchor="w"
        )
    ...
    def _build_settings(self):
        ...
        ctk.CTkLabel(logo_frame, text="Логотип:").grid(
            row=0, column=0, sticky="w", pady=4
        )
        self.logo_label = ctk.CTkLabel(
            logo_frame,
            text=os.path.basename(self.logo_path) if self.logo_path else "Не выбран",
            width=160,
            anchor="w",
        )
        ...
        ctk.CTkLabel(logo_frame, text="QR-код:").grid(
            row=1, column=0, sticky="w", pady=4
        )
        self.qr_label = ctk.CTkLabel(
            logo_frame,
            text=os.path.basename(self.qr_path) if self.qr_path else "Не выбран",
            width=160,
            anchor="w",
        )
        ...
        self.show_qr_var = ctk.BooleanVar(
            value=self.template_data.get("show_qr", False)
        )
        ctk.CTkCheckBox(
            logo_frame,
            text="Показывать QR-код",
            variable=self.show_qr_var,
            fg_color=self.colors["accent"],
            command=self.update_preview,
        ).grid(row=2, column=0, columnspan=2, sticky="w", pady=4)

        # Размер логотипа (ползунок)
        ctk.CTkLabel(logo_frame, text="Размер логотипа:").grid(
            row=3, column=0, sticky="w", pady=4
        )
        logo_size_frame = ctk.CTkFrame(logo_frame, fg_color="transparent")
        logo_size_frame.grid(
        ...
        self.logo_size_label = ctk.CTkLabel(
            logo_size_frame,
            text=f"{int(self.logo_size_slider.get())}pt",
            width=40,
        ...
        # Позиция логотипа
        ctk.CTkLabel(logo_frame, text="Позиция логотипа:").grid(
            row=4, column=0, sticky="w", pady=4
        )
        self.logo_pos_combo = ctk.CTkComboBox(
            logo_frame,
        ...
        row += 1
        ctk.CTkLabel(sf, text="Цвет акцента:").grid(
            row=row, column=0, sticky="w", pady=(6, 3)
        )
        ...
        self.color_preview = ctk.CTkFrame(
            color_frame, width=28, height=22, corner_radius=4, fg_color=current_color
        )
        ...
        for hex_c, _label in palette:
            btn = ctk.CTkButton(
                ...
                border_width=2,
                border_color=(
                    "#ffffff" if hex_c != current_color else self.colors["accent"]
                ),
        ...
        self.font_combo.set(
            current_font
            if current_font in ["Trebuchet MS", "Arial", "Times New Roman", "Helvetica"]
            else "Trebuchet MS"
        )
        ...
        self.margins_combo = ctk.CTkComboBox(
            sf,
            values=["6", "8", "10", "12", "15"],
            width=60,
            height=28,
        ...
        self.fields_container = ctk.CTkScrollableFrame(
            ff, fg_color="transparent", height=280
        )
        ...
        ctk.CTkLabel(add_frame, text="Добавить поле:", font=ctk.CTkFont(size=11)).pack(
            side="left", padx=(0, 8)
        )

        self.add_field_combo = ctk.CTkComboBox(
            add_frame,
            values=[FIELD_LABELS[k] for k in FIELD_LABELS if k not in current_fields],
            width=200,
            height=28,
        )
        ...
        hint_label = ctk.CTkLabel(
            ff,
            text="💡 Перетаскивайте поля за ⋮⋮ для изменения порядка",
        ...
        if self.act_type == "receipt":
            ctk.CTkLabel(
                text_card,
                text="📜 Условия ремонта",
                font=ctk.CTkFont(size=13, weight="bold"),
            ).pack(anchor="w", padx=12, pady=(8, 4))
            self.conditions_text = ctk.CTkTextbox(
                text_card, height=110, fg_color=self.colors["bg_tertiary"]
            )
            ...
            self.warranty_text = ctk.CTkTextbox(
                text_card, height=110, fg_color=self.colors["bg_tertiary"]
            )
            self.warranty_text.pack(fill="x", padx=12, pady=(0, 8))
            self.warranty_text.insert(
                "1.0", self.template_data.get("warranty_text", "")
            )
    ...
    def _create_field_item(self, field_key: str, index: int):
        ...
        frame = ctk.CTkFrame(
            self.fields_container, fg_color=self.colors["bg_secondary"], height=32
        )
        ...
        label = ctk.CTkLabel(
            frame,
            text=FIELD_LABELS.get(field_key, field_key),
            font=ctk.CTkFont(size=11),
            anchor="w",
        )
    ...
    def _bind_drag_events(self, frame, gripper, field_key):
        ...
        gripper.bind(
            "<ButtonPress-1>", lambda e: self._on_drag_start(e, frame, field_key)
        )
    ...
    def _update_add_field_combo(self):
        ...
        available = [FIELD_LABELS[k] for k in FIELD_LABELS if k not in current_fields]
    ...
    def sync_template_from_ui(self):
        ...
        if (
            hasattr(self, "_get_field_order")
            and hasattr(self, "field_widgets")
            and self.field_widgets
        ):
            ...
            self.template_data["fields"] = [
                k for k, v in self.field_vars.items() if v.get()
            ]
        if self.act_type == "receipt":
            self.template_data["conditions"] = self.conditions_text.get(
                "1.0", "end-1c"
            ).strip()
        else:
            self.template_data["warranty_text"] = self.warranty_text.get(
                "1.0", "end-1c"
            ).strip()
    ...
    def _apply_quick_color(self, hex_color: str):
        ...
        for btn, h in self._color_buttons:
            btn.configure(
                border_color=(self.colors["accent"] if h == hex_color else "#ffffff")
            )
        self.update_preview()
    ...
    def reset_to_defaults(self):
        ...
        if not messagebox.askyesno(
            "Сброс",
            "Сбросить настройки акта к стандартным?\nТекущие изменения будут потеряны.",
        ):
            return
        # Загружаем дефолт
    ...
    def _reload_ui(self):
        ...
        for btn, h in self._color_buttons:
            btn.configure(
                border_color=(self.colors["accent"] if h == color else "#ffffff")
            )
        # Логотип
    ...
    def update_preview(self):
        ...
        try:
            ...
            logger.exception(f"Ошибка предпросмотра: {e}")
    ...
    def export_to_pdf(self):
        ...
        try:
            ...
            gen = ActPDFGenerator(
                self._build_company_info(), template_data=self.template_data
            )
            device = self._build_demo_device()
            if self.act_type == "completion":
            ...
            messagebox.showerror(
                "Ошибка", "Установите reportlab: pip install reportlab"
            )
        except Exception as e:
    ...
    def print_pdf(self):
        ...
        try:
            ...
            try:
                ...
                logger.exception(f"Прямая печать не удалась: {pe}")
                try:
                    if _sys.platform == "win32":
            ...
            if printed:
                ...
                messagebox.showwarning(
                    "Печать",
                    f"PDF создан, но отправка на печать не удалась.\nФайл: {tmp_pdf}",
                )
        except ImportError:
            messagebox.showerror(
                "Ошибка", "Установите reportlab: pip install reportlab"
...
class ReportEditor(ctk.CTkToplevel):
    ...
    def __init__(
        self, parent, colors: dict, template_type: str = "receipt", settings=None
    ):
        ...
        self.initial_tab = (
            template_type if template_type in ("receipt", "completion") else "receipt"
        )
    ...
    def _fit_window(self, default_w: int, default_h: int, min_w: int, min_h: int):
        ...
        fit_window_to_screen(self, default_w, default_h, min_w, min_h, width_ratio=0.96)

    def active_panel(self) -> ActPanel:
        """Возвращает панель текущей активной вкладки."""
        tab_name = self.tabview.get()
        for act_type, name in (
            ("receipt", "📄 Акт приёма"),
            ("completion", "🔧 Акт выполненных работ"),
        ):
            if name == tab_name:
    ...
    def create_widgets(self):
        ...
        toolbar = ctk.CTkFrame(
            self, fg_color=self.colors["bg_secondary"], height=50, corner_radius=0
        )
        ...
        ModernButton(
            ...
            variant="success",
            text="📄 Экспорт PDF (A5)",
            command=self.export_active_to_pdf,
            width=150,
        ).pack(side="left", padx=2)
        ModernButton(
            btn_frame,
            ...
            variant="success",
            text="📋 2 на лист A4",
            command=self.export_dual_from_editor,
            width=130,
        ).pack(side="left", padx=2)
        ModernButton(
            btn_frame,
        ...
        target_name = (
            "📄 Акт приёма"
            if self.initial_tab == "receipt"
            else "🔧 Акт выполненных работ"
        )
    ...
    def _on_tab_changed(self):
        ...
        try:
            ...
            logger.exception(f"Ошибка обновления вкладки: {e}")
    ...
    def import_settings(self):
        ...
        try:
            file_path = filedialog.askopenfilename(
                title="Выберите файл настроек", filetypes=[("JSON", "*.json")]
            )
            if not file_path:
                return
    ...
    def export_dual_from_editor(self):
        ...
        try:
            ...
            gen = ActPDFGenerator(
                panel._build_company_info(), template_data=panel.template_data
            )
            ...
            ok = gen.generate_dual_pdf(
                ...
                device,
                device,
                act_type1=panel.act_type,
                act_type2=panel.act_type,
            )
            if ok:
    ...
    def _close_with_geometry(self):
        ...
        close_dialog_with_geometry(self, self.settings, "report_editor")
