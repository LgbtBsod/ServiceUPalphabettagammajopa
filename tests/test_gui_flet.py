...
class _FakePage:
    ...
    def run_task(self, coro_fn, *args, **kwargs):
        ...
        уже крутящемся asyncio-цикле Flet — здесь такого цикла нет, зато
        синхронный тест, вызвавший код, ведущий сюда, ждёт РЕЗУЛЬТАТА
        синхронно. asyncio.run() и даёт то же самое поведение "к моменту
        возврата всё уже случилось" (см. _print_act() — все его тесты
        писались для синхронной версии и полагаются именно на это).
        Тесты фич, которым нужен НАСТОЯЩИЙ конкурентный async (например,
        FilePicker.pick_files(), сам по себе await'ящий ответ реального
        Flet-клиента) по-прежнему не используют _FakeApp — см. отдельный
        интеграционный live-прогон в браузере."""
        import asyncio
