...
class ClientsAnalyticsQuery(AnalyticsQuery):
    ...
    def execute(self, session) -> dict[str, Any]:
        ...
        new_clients_today = (
            session.query(Client).filter(Client.created_at >= today).count()
        )

        # Топ клиентов по количеству заказов
        top_clients = (
            session.query(
                Client.name, Client.phone, func.count(Order.id).label("order_count")
            )
...
class AnalyticsService(BaseService):
    ...
    def generate_report(
        ...
        def _generate() -> AnalyticsReport:
            ...
            data = self._collect_report_data(
                report_type, filters, period_start, period_end
            )
    ...
    def _collect_report_data(
        ...
        try:
            ...
            self.logger.exception(f"Error collecting report data: {e}")
            return {}
