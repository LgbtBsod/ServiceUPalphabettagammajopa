...
> **⚠️ УСТАРЕЛО (см. AUDIT_REPORT_v21.md):** Тесты здесь (включая
> `test_core_application_initialization`) проверяли `core.application` —
> модуль удалён, никогда не был живым путём приложения. Актуальные тесты —
> `tests/` (реальная Kernel/facade/plugin-архитектура, `pytest` без флагов).
